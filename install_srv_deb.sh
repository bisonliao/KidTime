...
if ! command -v apt-get >/dev/null 2>&1; then
  echo "apt-get was not found. Use install_srv_rpm.sh for Alibaba/RHEL-like systems." >&2
  exit 1
fi
...
if ! "$PYTHON_BIN" -c 'import sys; raise SystemExit(0 if sys.version_info >= (3, 8) else 1)'; then
  echo "Python 3.8+ is required. Install a newer python3 package, then rerun." >&2
  exit 1
fi
...
if command -v ufw >/dev/null 2>&1 && sudo ufw status | grep -q "Status: active"; then
  echo "ufw is active. Open the port if clients connect directly:"
